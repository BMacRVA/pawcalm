...
export default function Celebration({ type, dogName, onClose }: { type: CelebrationType; dogName: string; onClose: () => void }) {
  ...
  const celebrations = {
    first_session: {
      emoji: '🎉',
      title: 'You did it!',
      message: `You completed your first session with ${dogName}. That's the hardest part — showing up.`
    },
    streak_3: {
      emoji: '🔥',
      title: '3-Day Streak!',
      message: `Three days in a row! You're building a habit. ${dogName} is learning the routine.`
    },
    streak_7: {
      emoji: '🏆',
      title: 'One Week Streak!',
      message: `Most people quit by now. Not you. ${dogName} is lucky to have you.`
    },
    first_great: {
      emoji: '⭐',
      title: `${dogName} had a great session!`,
      message: `This is what progress looks like. Keep it up!`
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center">
        ...
        <button onClick={onClose} className="bg-amber-600 hover:bg-amber-700 text-white px-6 py-3 rounded-xl font-semibold">
          Keep Going →
        </button>
      </div>
    </div>
